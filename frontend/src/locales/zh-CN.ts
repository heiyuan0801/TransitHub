  ...
  admin: {
    ...
    connectionHealth: {
      ...
      embed: {
        ...
        standaloneTitle: '在线检测嵌入',
        standaloneDescription: '独立配置在线模型检测，并生成可以放到任意网站的嵌入页面。嵌入页面只返回检测结果，不会暴露 API Key。',
        standaloneSettings: '嵌入设置',
        openAccessHint: '不设置来源白名单，生成的页面可以在任意站点打开；访问令牌仍会保护接口。',
        generatedUrl: '嵌入页面地址',
        generatedCode: 'iframe 代码',
        copyUrl: '复制地址',
        copyFailed: '复制失败，请手动复制。',
        generated: '已生成新的嵌入令牌',
        loading: '正在加载嵌入设置...',
        interval: '实际检测/刷新间隔（秒）',
        intervalHint: '每次刷新都会重新调用一次已配置的 API，范围 10–3600 秒。',
        ...
        copy: '复制嵌入代码',
        copied: '嵌入代码已复制',
        customTitle: '自定义 API 检测',
        customDescription: '输入兼容 OpenAI Chat Completions 的地址、密钥和模型。密钥只在服务端加密保存，不会出现在嵌入页面。',
        customEnabled: '启用自定义检测',
        customBaseUrl: 'API Base URL',
        customBaseUrlPlaceholder: 'https://api.example.com',
        customApiKey: 'API Key',
        customApiKeyPlaceholder: '留空表示保留已保存的 Key',
        customModel: '模型名称',
        customModelPlaceholder: '例如 gpt-4o-mini',
        customProvider: '模型类型',
        ...
        encryptionUnavailable: '服务器未配置 SMTP_ENCRYPTION_KEY，无法安全保存 API Key。'
      },
      events: {
        title: '最近探活与远端动作',
        empty: '暂无事件记录。',
